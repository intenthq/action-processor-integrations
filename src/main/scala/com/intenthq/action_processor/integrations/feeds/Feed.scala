...
trait Feed[I, O] {
  ...
  protected def inputStream(feedContext: FeedContext[IO]): fs2.Stream[IO, I]
  protected def transform(feedContext: FeedContext[IO]): fs2.Pipe[IO, I, O]

  protected def serialize(o: O): Array[Byte]
  protected val serialization: fs2.Pipe[IO, O, Array[Byte]] = _.map(serialize)
